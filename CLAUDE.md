...
**編集を始める前に `git fetch` / `git pull --rebase origin master` を実行すること。**

管理画面の Vibe Coding（`/admin/vibe` → GitHub Actions → master へ push）により、
**このセッションが関与しないコミットが master に積まれる。** ユーザーがブラウザから直接
機能を修正できるため、ローカルが最新である保証がない。

- push が rejected されたら、まず **Vibe 経由の正規のコミット**を疑う（事故ではない）
- 長時間の作業では、**push の直前にもう一度 fetch** する
- コンフリクトしたら rebase で自分の変更を上に積む
